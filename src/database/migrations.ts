...
/**
 * Interface para rastrear migraciones ejecutadas
 */
...
export class MigrationRunner {
   ...
   */
  private async inicializarTablaMigraciones(
    client: PoolClient
  ): Promise<void> {
   ...
   */
  private async obtenerMigracionesEjecutadas(
    client: PoolClient
  ): Promise<string[]> {
    try {
      const resultado = await client.query(
        'SELECT nombre FROM _migraciones WHERE estado = $1 ORDER BY id_migracion ASC',
        ['completada']
      );
   ...
   */
  private async leerMigracionesDisponibles(): Promise<
    Array<{ numero: number; nombre: string; contenido: string }>
  > {
    try {
      ...
      const migraciones = archivos
        ...
        .map((archivo) => {
          ...
          if (!match) {
            throw new Error(
              `Formato de archivo de migración inválido: ${archivo}. Usar: NNN_nombre.sql`
            );
          }
  ...
  private async registrarMigracion(
    ...
    try {
      await client.query(
        `INSERT INTO _migraciones (nombre, contenido_sql, estado)
         VALUES ($1, $2, $3)
         ON CONFLICT (nombre) DO NOTHING`,
        [nombre, contenido, 'completada']
      );
    } catch (error) {
  ...
  async up(): Promise<void> {
    ...
    try {
      ...
      const pendientes = disponibles.filter(
        (m) => !ejecutadas.includes(m.nombre)
      );
   ...
   */
  async down(pasos: number = 1): Promise<void> {
    const client = await this.pool.connect();
    ...
    try {
      ...
      const resultado = await client.query(
        `SELECT nombre, contenido_sql FROM _migraciones
      ...
      logger.warn(
        'Considere usar herramientas como Flyway o Liquibase para rollbacks automáticos'
      );
      ...
      for (const row of resultado.rows) {
        ...
        await client.query(
          `UPDATE _migraciones SET estado = $1 WHERE nombre = $2`,
          ['revertida', row.nombre]
        );
      }
      ...
      logger.info(
        `✓ ${resultado.rows.length} migraciones marcadas como revertidas`
      );
    } catch (error) {
      await client.query('ROLLBACK');
  ...
  async status(): Promise<void> {
    ...
    try {
      ...
      const resultado = await client.query(
        `SELECT nombre, fecha_ejecucion, estado FROM _migraciones
      ...
      resultado.rows.forEach((row) => {
        ...
        console.log(
          `│ ${row.nombre.padEnd(29)} │ ${fecha} │ ${estado.padEnd(7)} │`
        );
      });
  ...
  async validar(): Promise<boolean> {
    try {
      if (!fs.existsSync(this.migrationsDir)) {
        logger.warn(
          `⚠️  Directorio de migraciones no existe: ${this.migrationsDir}`
        );
        return false;
      }
