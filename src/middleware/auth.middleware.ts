...
export const authMiddleware = async (
  ...
  try {
    ...
    try {
      ...
      const mensaje =
        error instanceof Error && error.message === 'Token expirado'
    ...
    // Paso 5: Adjuntar datos al request
    req.usuario_id = decoded.usuario_id;
    req.email = decoded.email;
    req.token = token;

    // Paso 6: Obtener rol del usuario desde la BD
    try {
      const pool = getPool();
      const { rows } = await pool.query(
        'SELECT rol FROM usuarios WHERE usuario_id = $1',
        [decoded.usuario_id]
      );
      req.rol = rows[0]?.rol ?? 'usuario';
    } catch {
      req.rol = 'usuario';
    }

    logger.debug(`✓ Auth successful for user: ${decoded.usuario_id} (rol: ${req.rol})`);
...
export const authRateLimitMiddleware = (
  ...
  try {
    ...
    let intentos = loginAttempts.get(identificador as string);

    // Si no hay intentos previos o la ventana expiró, crear nuevo contador
    if (!intentos || ahora - intentos.lastAttempt > ATTEMPT_WINDOW) {
      loginAttempts.set(identificador as string, { count: 1, lastAttempt: ahora });
      next();
      return;
    }
    ...
    if (intentos.count > ATTEMPT_LIMIT) {
      logger.warn(
        `🚫 Too many auth attempts from IP: ${identificador}. Count: ${intentos.count}`
      );
      res.status(429).json({
 ...
 */
export function inicializarLimpiezaLoginAttempts(): void {
  setInterval(() => {
    const ahora = Date.now();
    let limpiados = 0;
