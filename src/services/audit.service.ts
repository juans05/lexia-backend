...
export class AuditService {
   ...
   */
  async registrarEvento(dto: RegistrarEventoDto): Promise<string> {
    try {
      const query = `
      ...
      logger.debug(
        `✓ Evento auditado: ${dto.tipo_evento} (${dto.resultado || 'exitoso'})`
      );
   ...
   * @param ipAddress - IP del cliente
   */
  async registrarLogout(usuarioId: string, ipAddress?: string): Promise<void> {
    try {
      await this.registrarEvento({
        usuario_id: usuarioId,
        tipo_evento: 'logout',
   ...
   * @param usuarioId - ID del usuario
   */
  async registrarCambioContrasena(usuarioId: string): Promise<void> {
    try {
      await this.registrarEvento({
        usuario_id: usuarioId,
   ...
   */
  async registrarEliminacionCuenta(usuarioId: string, email: string): Promise<void> {
    try {
      await this.registrarEvento({
        usuario_id: usuarioId,
   ...
   */
  async obtenerEventosDelUsuario(
    usuarioId: string,
    limite: number = 50,
    offset: number = 0
    ...
    try {
      ...
      return resultado.rows as EventoAuditoria[];
    } catch (error) {
   ...
   */
  async obtenerEventosPorFecha(desde: Date, hasta: Date): Promise<EventoAuditoria[]> {
    try {
      const query = `
        SELECT * FROM auditoría_logs
   ...
   */
  async limpiarLogsAntiguos(diasRetención: number = 90): Promise<number> {
    try {
      const query = `
      ...
      if (resultado.rowCount && resultado.rowCount > 0) {
        logger.info(
          `🧹 ${resultado.rowCount} registros de auditoría antiguos eliminados`
        );
      }
 ...
 */
export function inicializarLimpiezaAutomaticaAudit(): void {
