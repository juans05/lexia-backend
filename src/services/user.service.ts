...
export class UserService {
  ...
  async crearUsuario(dto: CrearUsuarioDto): Promise<Usuario> {
    ...
    try {
      ...
      const usuario = resultado.rows[0] as Usuario;
  ...
  async guardarCodigoVerificacion(usuarioId: string, codigo: string, expira: Date): Promise<void> {
    try {
      await this.pool.query(
        `UPDATE usuarios SET codigo_verificacion = $1, codigo_verificacion_expira = $2 WHERE usuario_id = $3`,
  ...
  async validarCodigoVerificacion(email: string, codigo: string): Promise<string | null> {
    try {
      const resultado = await this.pool.query(
        `SELECT usuario_id, codigo_verificacion, codigo_verificacion_expira
         FROM usuarios WHERE email = $1 AND es_activo = TRUE`,
        [email.toLowerCase()]
      );
      ...
      if (
        row.codigo_verificacion !== codigo ||
  ...
  async limpiarCodigoVerificacion(usuarioId: string): Promise<void> {
    try {
      await this.pool.query(
        `UPDATE usuarios SET codigo_verificacion = NULL, codigo_verificacion_expira = NULL WHERE usuario_id = $1`,
        [usuarioId]
      );
    } catch (error) {
   ...
   * @param usuarioId - UUID del usuario
   */
  async marcarTelefonoVerificado(usuarioId: string): Promise<void> {
    try {
      const query = `
        UPDATE usuarios
   ...
   */
  async cambiarPassword(usuarioId: string, newPasswordHash: string): Promise<void> {
    try {
      const query = `
        UPDATE usuarios
   ...
   * @param usuarioId - UUID del usuario
   */
  async desactivarCuenta(usuarioId: string): Promise<void> {
    try {
      const query = `
        UPDATE usuarios
   ...
   * @param usuarioId - UUID del usuario
   */
  async actualizarUltimaActividad(usuarioId: string): Promise<void> {
    try {
      const query = `
        UPDATE usuarios
   ...
   */
  private async crearEstadisticasIniciales(
    client: any,
    usuarioId: string
  ): Promise<void> {
    try {
  ...
  async obtenerUsuariosActivos(
    ...
    try {
      ...
      return resultado.rows as Usuario[];
    } catch (error) {
