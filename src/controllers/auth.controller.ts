...
export const login = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const esValida = await authService.verifyPassword(
      value.password,
    ...
    const { accessToken, refreshToken, expiresIn } =
      await authService.generateTokens(usuario.usuario_id, usuario.email);
...
export const verifyEmail = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (!usuario) throw new Error('Usuario no encontrado tras verificación');
